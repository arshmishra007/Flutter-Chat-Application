...
class _MyChatState extends State<MyChat> {
  ...
  Widget build(BuildContext context) {
    ...
    var sendIcon = Icon(Icons.send, color: Colors.white);
    var UserIcon = IconButton(
    ...
    var appbar = AppBar(
      leading: logo,
      title: Text("Chats"),
      actions: <Widget>[HomeButton, UserIcon],
                  ...
                  StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      for (var d in msg) {
                        // print(d.data()['sender']);
                        var msgText =
                            (d.data == null) ? null : d.data()['text'];
                        var msgSender =
                            (d.data == null) ? null : d.data()['sender'];
                        var msgtime =
                            (d.data == null) ? null : d.data()['time'];
                        var sendshape = RoundedRectangleBorder(
                            borderRadius: BorderRadius.only(
                                topLeft: Radius.circular(20),
                                topRight: Radius.circular(20),
                                bottomLeft: Radius.circular(20)));
                        var recvshape = RoundedRectangleBorder(
                            borderRadius: BorderRadius.only(
                                topLeft: Radius.circular(20),
                                topRight: Radius.circular(20),
                        ...
                        var msgWidget = Padding(
                            ...
                            child: Column(
                              crossAxisAlignment: (signInUser == msgSender)
                              ...
                              children: <Widget>[
                                MaterialButton(
                                  elevation: 30,
                                  shape: (signInUser == msgSender)
                                      ? sendshape
                                      : recvshape,
                                  color: Colors.grey,
                                  disabledColor: (signInUser == msgSender)
                                      ? Colors.red
                                      : Colors.blue,
